...
func (cr *clusterRouter) GetReleaseV2(c *gin.Context) {
...
func (cr *clusterRouter) ListReleasesV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err      error
		helmMeta types.PixiuObjectMeta
...
func (cr *clusterRouter) InstallReleaseV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err         error
		helmMeta    types.PixiuObjectMeta
		releaseForm types.Release
	)
	if err = httputils.ShouldBindAny(c, &releaseForm, &helmMeta, nil); err != nil {
		httputils.SetFailed(c, r, err)
		return
	}

	if r.Result, err = cr.c.Cluster().Helm(helmMeta.Cluster).ReleasesV2(helmMeta.Namespace).InstallRelease(c, &releaseForm); err != nil {
		httputils.SetFailed(c, r, err)
		return
	}
...
func (cr *clusterRouter) UninstallReleaseV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err      error
		helmMeta types.PixiuObjectMeta
...
func (cr *clusterRouter) UpgradeReleaseV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err         error
		helmMeta    types.PixiuObjectMeta
	...
	if r.Result, err = cr.c.Cluster().Helm(helmMeta.Cluster).Releases(helmMeta.Namespace).UpgradeRelease(c, &releaseForm); err != nil {
		httputils.SetFailed(c, r, err)
		return
	}
...
func (cr *clusterRouter) GetReleaseHistoryV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err      error
		helmMeta types.PixiuObjectMeta
...
func (cr *clusterRouter) RollbackReleaseV2(c *gin.Context) {
	r := httputils.NewResponse()
	var (
		err           error
		helmMeta      types.PixiuObjectMeta
		reversionMeta types.ReleaseHistory
	)
	if err = httputils.ShouldBindAny(c, nil, &helmMeta, &reversionMeta); err != nil {
		httputils.SetFailed(c, r, err)
		return
	}

	if err = cr.c.Cluster().Helm(helmMeta.Cluster).Releases(helmMeta.Namespace).RollbackRelease(c, helmMeta.Name, reversionMeta.Version); err != nil {
		httputils.SetFailed(c, r, err)
		return
	}
